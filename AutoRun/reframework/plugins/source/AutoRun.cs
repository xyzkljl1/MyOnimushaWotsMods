...
// Source blob SHA-1: 52f8d70fa0c3a7aa9f839958a404e325366aedcb
// Source commit: a06a7f10c54d07cbf453a337efd9726d9672e972
...
public sealed class AutoRun : ModBase
{
    ...
    public static PreHookResult BeforeDashKeepUpdate(Span<ulong> args)
    {
        try
        {
            ...
            var manager =
                GetHookArgument<app.cPlayerActionSelector.cDashKeepManager>(args, 1);
            var character = GetHookArgument<app.cPlayerCharacterEntity>(args, 2);
            var controller = GetHookArgument<app.cPlayerDefaultInputControllerEntity>(args, 3);
            if (manager is null || character is null || controller is null)
            {
                ResetSprint();
                return PreHookResult.Continue;
            }
            ...
            var playerControllerAddress =
                ...
                ?.GetAddress() ?? 0;
            var controllerAddress = (controller as IProxyable)?.GetAddress() ?? 0;
            ...
            var canSprint =
                controller.CommandResult?.IsEnableInputWorldDirL == true &&
                (moveType == app.PlayerDef.MOVE_TYPE.RUN ||
                 moveType == app.PlayerDef.MOVE_TYPE.DASH);
            if (!canSprint)
            {
                ResetSprint();
                manager.IsDashKeep = false;
            ...
            manager.IsDashKeep = now - _runStartedAt >= Instance._sprintDelay.Value;
            return PreHookResult.Skip;
        }
        catch (Exception exception)
        {
            ResetSprint();
            Instance.LogErrorOnce("DASH state update failed", exception);
            return PreHookResult.Continue;
        }
    }

    private static void ResetSprint() => _runStartedAt = -1.0f;
}
